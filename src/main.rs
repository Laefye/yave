...
async fn main() {
    ...
    let vm = VirtualMachine::load("debug/vmtest.toml").expect("Failed to load VM config");
    ...
    qmp.invoke(InvokeCommand::set_vnc_password("12345678")).await.expect("Failed to set VNC password");
}
